      ...
      <el-table :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column prop="requestType" label="请求方式" width="90">
          <template #default="{ row }">
            <el-tag :type="methodColor(row.requestType)" size="small">{{ row.requestType }}</el-tag>
          </template>
        </el-table-column>
...
const formRef = ref()
const form = reactive({
  id: 0, suiteCode, methodName: '', requestType: 'GET', url: '', contentType: 'JSON', methodDesc: ''
})
const rules = {
...
function openAdd() {
  isEdit.value = false
  Object.assign(form, { id: 0, methodName: '', requestType: 'GET', url: '', contentType: 'JSON', methodDesc: '' })
  dialogVisible.value = true
}

function openEdit(row: any) {
  isEdit.value = true
  Object.assign(form, { id: row.id, methodName: row.methodName, requestType: row.requestType, url: row.url, contentType: row.contentType, methodDesc: row.methodDesc })
  dialogVisible.value = true
}
